...
def main(args, config):
    ...
    boxsize_x     = float(config["simulation"]["boxsize_x"])
    boxsize_y     = float(config["simulation"]["boxsize_y"])
    boxsize_z     = float(config["simulation"]["boxsize_z"])
    ...
    # Domaine
    xlin = jnp.linspace(0.5 * dx, boxsize_x - 0.5 * dx, Nx)
    ylin = jnp.linspace(0.5 * dy, boxsize_y - 0.5 * dy, Ny)
    zlin = jnp.linspace(0.5 * dz, boxsize_z - 0.5 * dz, Nz)
    ...
    rho, vx, vy, vz, P, Teq, phi, Energy = inital_condition(IC, X, Y, Z, gamma)
    ...
    for i in range(max_steps):
        ...
        Mass, Momx, Momy, Momz, Energy, dt, rho = update(
            Mass, Momx, Momy, Momz, Energy, dx, dy, dz, gamma, courant_fac, phi, tau, Teq, grav=-1.0, cv=1.5
        )
