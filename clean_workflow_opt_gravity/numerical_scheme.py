...
def update(rho, Momx, Momy, Momz, Energy, dx, dy, dz, gamma, courant_fac, Bx, By, Bz):
    ...
    def get_flux(rho_L, rho_R, vx_L, vx_R, vy_L, vy_R, vz_L, vz_R, P_L, P_R, gamma, Bx_L, Bx_R, By_L, By_R, Bz_L, Bz_R, g = 1.0, direction_z=0):
        ...
        u_star = 0.5 * (vx_L + vx_R) - 0.5 * (Pmag_R - Pmag_L - Mz * direction_z) / aface
        ...
        w_star = 0.5 * (vz_L + vz_R) - 0.5 * (Rmag_R - Rmag_L - Mz * (1-direction_z)) / aface
        ...
        flux_Momx = jnp.where(
            u_star > 0, 
            u_star * vx_L * rho_L + p_star -0.5 * Mz * direction_z, 
            u_star * vx_R * rho_R + p_star -0.5 * Mz * direction_z)
        ...
        flux_Energy = jnp.where(
            u_star > 0, 
            u_star * en_L + p_star * u_star + q_star * v_star + r_star * w_star -0.5 * Mz * u_star * direction_z, 
            u_star * en_R + p_star * u_star + q_star * v_star + r_star * w_star -0.5 * Mz * u_star * direction_z
        )
    ...
    flux_rho_X, flux_Momx_X, flux_Momy_X, flux_Momz_X, flux_Energy_X, flux_Bx_X, flux_By_X, flux_Bz_X = get_flux(
        rho_XL, rho_XR, vx_XL, vx_XR, vy_XL, vy_XR, vz_XL, vz_XR, P_XL, P_XR, gamma, Bx_XL, Bx_XR, By_XL, By_XR, Bz_XL, Bz_XR, direction_z=0
    )

    flux_rho_Y, flux_Momy_Y, flux_Momx_Y, flux_Momz_Y, flux_Energy_Y, flux_By_Y, flux_Bx_Y, flux_Bz_Y = get_flux(
        rho_YL, rho_YR, vy_YL, vy_YR, vx_YL, vx_YR, vz_YL, vz_YR, P_YL, P_YR, gamma, By_YL, By_YR, Bx_YL, Bx_YR,  Bz_YL, Bz_YR, direction_z=0
    )

    flux_rho_Z, flux_Momz_Z, flux_Momy_Z, flux_Momx_Z, flux_Energy_Z, flux_Bz_Z, flux_By_Z, flux_Bx_Z = get_flux(
        rho_ZL, rho_ZR, vz_ZL, vz_ZR, vy_ZL, vy_ZR, vx_ZL, vx_ZR, P_ZL, P_ZR, gamma, Bz_ZL, Bz_ZR, By_ZL, By_ZR, Bx_ZL, Bx_ZR, direction_z=1
    )
